...
# data preprocessing for mmformer
# image - clip preprocessing
#       - {'pixel_values': [array(),...]} # [channel, height, width] 
# text  - text tokenizer, clip text tokenizer as default, if not specified. can also switch to another one
#       - {'input_ids': [1,2,3,...], 'attention_mask': [1,1,1,...]}
# mmformer takes pixel_values(FloatTensor) of shape [bs, channel, height, width] 
#            and input_ids(LongTensor)     of shape [bs, seq_len] # padding to max_seq_len due to different text len

def process_sample_clip(
    sample: Dict[str, Any],
    ...
    **kwargs,
    ...
    # image = Image.open(BytesIO(sample["jpg"]["bytes"])).convert("RGB")
    meta = sample["json"]
    # print(f"image size {image.size}")

    # conversations = conv_preprocess(source, meta, **kwargs)

    token_num_inputs, image_inputs, tokenized_example = {}, {}, {}
    image_inputs = processor.image_processor(images=image, return_tensors="pt")
    # clip image preprocessor returns pixel_values of shape [bs, channel, height, width]
    # image_grid_thw = image_inputs["image_grid_thw"]
    # image_token_num = (image_inputs['pixel_values'].shape[-2] // 14) * (image_inputs['pixel_values'].shape[-1] // 14)
    # token_num_inputs["image"] = [image_token_num]
    tokenized_example["input_ids"] = processor.tokenizer.encode(meta["caption"])    
    ...
    tokenized_example['attention_mask'] = torch.ones(tokenized_example["input_ids"].shape[-1])
    return [tokenized_example]


# modified by fangyan:
# remove the image preprocessing in the chat template
def process_sample(
    sample: Dict[str, Any],
    processor: "ProcessorMixin",
...
def main():
    ...
    processor = build_processor(args.model.clip_preprocessor)
