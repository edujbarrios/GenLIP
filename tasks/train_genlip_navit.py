...
from veomni.models.transformers.genlip import GenLIPNaViTImageProcessor
...
def process_sample(
    ...
    **kwargs,
    ...
    meta = sample['json']
    conversations = conv_preprocess(source, meta, **kwargs) # use mm_pretrain
    if len(conversations) == 0:
        raise ValueError(f"Sample {sample['__url__']} - {sample['__key__']} with no conversation data field.")
    ...
    if "jpg" in sample and len(sample['jpg']) > 0:
        try:
            image = Image.open(BytesIO(sample["jpg"]))
        except:
            image = None
    ...
    try:
        ...
        num_patches = image_inputs['image_grid_thw'].prod().item()
        token_num_inputs["image"] = [num_patches]
    except:
        raise ValueError(f"Sample {sample['__key__']} with no image.")

    try:
        tokenized_example = chat_template.encode_messages(conversations, token_num_inputs, no_special_tokens=True)
    except:
        print(f"sample has error when process conversation: {conversations}")
        return None
    ...
    if tokenized_example['image_mask'].sum() != num_patches:
    ...
    tokenized_example["position_ids"] = position_ids.squeeze()
...
class MyDataArguments(DataArguments):
    ...
    datasets_type: Literal["mapping", "iterable_wds", "iterable", "iterable_wds_api"] = field(
        default="mapping",
    ...
    max_sample_len: int = field(
        default=4096,
        metadata={"help": "Max total tokens count for native aspect ratio pretraining."},
    )
...
def main():
    ...
    logger.info_rank0(f"mm_config {args.data.mm_configs}")

    # todo: check correctness
    # directly setting image_processor is prior than re-init and overwriting
    if len(args.data.mm_configs.keys()) > 0:
        custom_image_processor = CustomImageProcessor()
        custom_image_processor = custom_image_processor.from_dict(args.data.mm_configs)
        processor.image_processor = custom_image_processor
    elif args.model.image_processor is not None:
    ...
    logger.info_rank0(f"Data Preprocessor:\n {processor}")
    ...
    transform = partial(
        ...
        max_len=args.data.max_sample_len,
    )
