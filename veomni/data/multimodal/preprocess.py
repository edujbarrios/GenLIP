...
def datacomp_1b_preprocess(conversations, **kwargs):
...
def mmpretrain_preprocess(conversations, **kwargs):
...
def mmpretrain_mixed_preprocess(json_content, **kwargs):
    # kw_maping = {""}
    role_mapping = {"human":"user", "gpt":"assistant", "user":"user", "assistant":"assistant"}
    if 'conversations' not in json_content.keys():
        caption = json_content['caption']
        constructed_conversation = [
            ["user", ("image", None), ("text", "Describe the image in detail.")],
            ["assistant", ("text", caption)],
        ]
        return constructed_conversation
    constructed_conversation = []
    conversations = json_content['conversations']
    # conversations = conversations[1:]
    if 'from' not in conversations[0].keys():
        new_converations = []
        for conv in conversations:
            for k, v in conv.items():
                new_converations.append(
                    {
                        'from': k,
                        'value': v,
                    }
                )
        conversations = new_converations
    if role_mapping[conversations[0]["from"]] != "user":  # Skip the first one if it is not from human
        conversations = conversations[1:]
    assert role_mapping[conversations[0]["from"]] == "user"
    
    conv_num = len(conversations) // 2
    for idx in range(conv_num):
        role0 = role_mapping[conversations[2 * idx]["from"]]
        role1 = role_mapping[conversations[2 * idx + 1]["from"]]
        if role0 == "user":
            val_user = conversations[2 * idx]["value"]
            val_asst = conversations[2 * idx + 1]["value"]
        else:
            val_user = conversations[2 * idx + 1]["value"]
            val_asst = conversations[2 * idx]["value"]
        if val_user is None:
            constructed_conversation.append(None)
        else:
            if "<image>" in val_user:
                val_user = val_user.replace("<image>", "")
                constructed_conversation.append(["user", ("image", None), ("text", val_user)])
            else:
                constructed_conversation.append(["user", ("text", val_user)])
            constructed_conversation.append(["assistant", ("text", val_asst)])
    return constructed_conversation
...
DATASETS = {
    ...
    "recap_datacomp_1b": recap_datacomp_1b_preprocess,
    "datacomp_1b": datacomp_1b_preprocess,
    "mmpretrain": mmpretrain_preprocess,
    "mmpretrain_mixed": mmpretrain_mixed_preprocess,
}
