...
class Qwen2VLTemplate(MultimodalChatTemplate):
    ...
    def image_pattern(self, token_num, no_special_tokens=False):
    ...
    def encode_messages(
        ...
        for message in conversations:
            ...
            for item in message[1:]:
                mm_type = item[0]
                if mm_type == "image":
                    if "image" in mm_num_tokens.keys() and len(mm_num_tokens[mm_type]) > 0:
                        content += self.image_pattern(mm_num_tokens[mm_type][0], no_special_tokens)
        ...
        for message in messages:
            ...
            if message["loss_mask"] == 1: # "assistant"
                labels += content_ids
            else:   # "human"
                labels += [IGNORE_INDEX] * len(content_ids)
        input_ids += self.eos
        ...
        input_image_mask = image_mask & input_mask # pure image tokens
        output_image_mask = image_mask & ~input_mask # image tokens from assistant
...
class CLIPPretrainTemplate(MultimodalChatTemplate):
    template_type = "pretrain"
    def __init__(self, tokenizer: PreTrainedTokenizer) -> None:
        super().__init__(tokenizer)
        self.image_pad = "<|image_pad|>"
        self.image_token_id = self.tokenizer.convert_tokens_to_ids(self.image_pad)
        # self.image_start_id = self.tokenizer.convert_tokens_to_ids("<|vision_start|>")
        self.eos = self.tokenizer.encode("<|endoftext|>", add_special_tokens=False)

        logger.info_rank0("CLIP will not truncate sequence when longer than [max_seq_lens].")
...
class VQAVLTemplate(MultimodalChatTemplate):
    template_type = "pretrain"
    def __init__(self, tokenizer: PreTrainedTokenizer) -> None:
        super().__init__(tokenizer)
        self.image_pad = "<|image_pad|>"
        self.image_token_id = self.tokenizer.convert_tokens_to_ids(self.image_pad)
        self.image_start_id = self.tokenizer.convert_tokens_to_ids("<|vision_start|>")
        self.eos = self.tokenizer.encode("<|im_end|>", add_special_tokens=False)
