    ...
    def __call__(self, features: Sequence[Dict[str, "torch.Tensor"]]) -> Dict[str, "torch.Tensor"]:
        ...
        try: 
            ...
            for idx, feature in enumerate(features):
                input_length = feature['input_ids'].shape[-1]
...
# def my_collate_fn(batches, max_seq_len):
#     """
#     Collates a list of batches, each processed by OmniDataCollatorWithPadding,
#     into a single, larger batch by applying padding and stacking.

#     Args:
#         batches (list): A list of dictionaries, where each dictionary is a batch.

#     Returns:
#         dict: A single dictionary representing the combined batch.
#     """
...
#             if key in batch:
#                 final_batch[key].append(batch[key])

#     for key in final_batch.keys():
#         tensors_to_process = final_batch[key]
...
#         if key in padding_features:
#             pad_value = padding_features.get(key, 0)
            
#             if key == "position_ids" and tensors_to_process[0].dim() == 3:
#                 pad_list = [item.squeeze(0).transpose(0, 1) if item.dim() > 1 else item for item in tensors_to_process]
#                 final_batch[key] = pad_sequence(pad_list, batch_first=True, padding_value=pad_value)
#                 final_batch[key] = final_batch[key].transpose(1, 2)
#             else:
#                 final_batch[key] = pad_sequence([item.squeeze(0) if item.dim() > 1 else item for item in tensors_to_process], batch_first=True, padding_value=pad_value)

#         elif key in concat_features:
#             final_batch[key] = torch.cat(tensors_to_process, dim=0)

#         else:
#             final_batch[key] = default_collate(tensors_to_process)
                ...
                for tensor in pad_list:
                    if tensor.size(0) < max_seq_len:
                        ...
                        if tensor.dim() == 1:
                            ...
                            padding = torch.full((pad_size, tensor.size(1)), pad_value, dtype=tensor.dtype, device=tensor.device)
                        tensor = torch.cat([tensor, padding], dim=0)
                    elif tensor.size(0) > max_seq_len:
                        tensor = tensor[:max_seq_len]
                    padded_tensors.append(tensor)
                final_batch[key] = torch.stack(padded_tensors)
                final_batch[key] = final_batch[key].transpose(1, 2)
            else:
                # Pad to max_seq_len for other features
                padded_tensors = []
                for item in tensors_to_process:
                    tensor = item.squeeze(0) if item.dim() > 1 else item
                    if tensor.size(0) < max_seq_len:
                        pad_size = max_seq_len - tensor.size(0)
                        if tensor.dim() == 1:
                            padding = torch.full((pad_size,), pad_value, dtype=tensor.dtype, device=tensor.device)
                        else:
                            padding = torch.full((pad_size,) + tensor.shape[1:], pad_value, dtype=tensor.dtype, device=tensor.device)
                        tensor = torch.cat([tensor, padding], dim=0)
                    elif tensor.size(0) > max_seq_len:
                        tensor = tensor[:max_seq_len]
                    padded_tensors.append(tensor)
