...
def build_checkpointer(
    ...
    Args:
        ...
        ckpt_manager (str, optional): checkpoint manager. Defaults to "bytecheckpoint".
            ...
            native: torch native checkpoint manager, torch.save()
    Raises:
    ...
    if ckpt_manager == "bytecheckpoint":
        ...
        if dist_backend not in ["ddp", "fsdp1", "fsdp2"]:
            raise ValueError(
                f"Unsupported distributed backend: {dist_backend} for native checkpoint manager, supported modes are: ddp, fsdp1, fsdp2"
            )
        Checkpointer = NativeCheckpointer(dist_backend=dist_backend)
    else:
        raise ValueError(f"Unknown checkpoint manager: {ckpt_manager}, supported modes are: bytecheckpoint, dcp, native")
...
class NativeCheckpointer(CheckpointerBase):
    def __init__(self, dist_backend: str = "fsdp1") -> None:
        self.dist_backend = dist_backend

        assert self.dist_backend in [
            "ddp",
            "fsdp1",
        ], f"Unsupported distributed backend {self.dist_backend}, supported modes are: ddp, fsdp1"
    ...
    def save(
        ...
        """
        Saves training state using native PyTorch checkpointing
        """
        checkpoint_dir = self.create_checkpoint_dir(path, global_steps)

        if self.dist_backend == "ddp":
            self.save_ddp(checkpoint_dir, state)
        elif self.dist_backend == "fsdp1":
            self.save_fsdp(checkpoint_dir, state)

        self.save_extra_state(checkpoint_dir, state)
        logger.info_rank0(f"Saved checkpoint to {checkpoint_dir}")
    ...
    def load(
        ...
        """
        Loads training state from native checkpoint
        Maintains original state structure
        """
        if self.dist_backend == "ddp":
            self.load_ddp(path, state)
        elif self.dist_backend == "fsdp1":
            self.load_fsdp(path, state)

        self.load_extra_state(path, state)
        logger.info_rank0(f"Loaded checkpoint from {path}")
        return state

    # Helper methods
    def create_checkpoint_dir(self, base_path: str, steps: Optional[int]) -> str:
        dir_name = f"{base_path}/global_step_{steps}" if steps else base_path
        os.makedirs(dir_name, exist_ok=True)
        return dir_name

    def save_ddp(self, checkpoint_dir: str, state: Dict[str, Any]) -> None:
        if dist.get_rank() != 0:
            return

        model_path = os.path.join(checkpoint_dir, _MODEL_DIR, "model.pt")
        os.makedirs(os.path.dirname(model_path), exist_ok=True)
        torch.save(state["model"].state_dict(), model_path)

        if "optimizer" in state:
            optim_path = os.path.join(checkpoint_dir, _OPTIMIZER_DIR, "optimizer.pt")
            os.makedirs(os.path.dirname(optim_path), exist_ok=True)
            torch.save(state["optimizer"].state_dict(), optim_path)

        dist.barrier()

    def save_fsdp(self, checkpoint_dir: str, state: Dict[str, Any]) -> None:
        ...
        if "optimizer" in state:
            if dist.get_rank() == 0:
                optim_path = os.path.join(checkpoint_dir, _OPTIMIZER_DIR, "optimizer.pt")
                full_osd = FSDP.full_optim_state_dict(model, state["optimizer"])
                os.makedirs(os.path.dirname(optim_path), exist_ok=True)
                torch.save(full_osd, optim_path)
            dist.barrier()

    def load_fsdp(self, path: str, state: Dict[str, Any]) -> None:
        from torch.distributed.fsdp import FullStateDictConfig, StateDictType
        from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
        ...
        with FSDP.state_dict_type(model, StateDictType.FULL_STATE_DICT, cfg):
            if dist.get_rank() == 0:
                model_path = os.path.join(path, _MODEL_DIR, "model.pt")
        ...
        if "optimizer" in state:
            if dist.get_rank() == 0:
                optim_path = os.path.join(path, _OPTIMIZER_DIR, "optimizer.pt")
                full_osd = torch.load(optim_path)
                sharded_osd = FSDP.shard_full_optim_state_dict(full_osd, model)
                state["optimizer"].load_state_dict(sharded_osd)
            dist.barrier()

    def save_extra_state(self, checkpoint_dir: str, state: Dict[str, Any]) -> None:
        if "extra_state" not in state:
            return

        extra_dir = os.path.join(checkpoint_dir, _EXTRA_STATE_DIR)
        os.makedirs(extra_dir, exist_ok=True)
        rank_suffix = _EXTRA_STATE_FORMAT.format(dist.get_rank())
        torch.save(state["extra_state"], os.path.join(extra_dir, rank_suffix))

    def load_extra_state(self, path: str, state: Dict[str, Any]) -> None:
        extra_path = os.path.join(path, _EXTRA_STATE_FORMAT.format(dist.get_rank()))
        if os.path.exists(extra_path):
            state["extra_state"] = torch.load(extra_path)
