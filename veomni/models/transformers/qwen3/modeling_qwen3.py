...
_CONFIG_FOR_DOC = "Qwen3_Config"

# implement based on this veomni Qwen3 implementation.
# we add gated attention in this version for controlled experiment.
class Qwen3VisionEmbedding(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor, grid_thw: torch.Tensor = None) -> torch.Tensor:
        ...
        if patch_embeds.shape[-1] * patch_embeds.shape[-2] == 1: # for navit
            patch_embeds = patch_embeds.squeeze()
        else:
            patch_embeds = patch_embeds.flatten(2).transpose(1, 2) # for fixed resolution pretraining
...
# use the same interleaved mrope implementation, will can be implemented in the following code.
...
class Qwen3Attention(nn.Module):
    ...
    def __init__(self, config: Qwen3_Config, layer_idx: int):
        ...
        if self.gated_attention:
            # predict both q and gated scores
            self.g_proj = nn.Linear(
                config.hidden_size, config.num_attention_heads * self.head_dim, bias=config.attention_bias
            ) # will be re-initialized and trained
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        hidden_shape = (*input_shape, -1, self.head_dim) # [bs, q_len, n, head_dim]

        if gated_attention:
            gate_score = self.g_proj(hidden_states).view(hidden_shape) # [bs, q_len, n, head_dim]
        ...
        if use_flex_attn:
            padlen = calculate_pad_length(query_states.shape[2], flex_attn_args.get("max_seqlen", self.config.max_position_embeddings))
...
class Qwen3LayerScale(nn.Module):
    def __init__(self, config: Qwen3_Config, inplace=False):
        super().__init__()
...
class Qwen3DropPath(nn.Module):
    def __init__(self, drop_prob: float = 0.0, scale_by_keep: bool = True):
        super().__init__()
        self.drop_prob = drop_prob
        self.scale_by_prob = scale_by_keep
...
class Qwen3DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen3_Config, layer_idx: int):
        ...
        self.layer_scale1 = Qwen3LayerScale(config, inplace=True) if config.ls_init_value > 0.0 else nn.Identity()
        self.layer_scale2 = Qwen3LayerScale(config, inplace=True) if config.ls_init_value > 0.0 else nn.Identity()
        self.drop_path = Qwen3DropPath(config.drop_path) if config.drop_path > 0.0 else nn.Identity()
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def __init__(self, config: Qwen3_Config):
        ...
        self.vision_embeddings = Qwen3VisionEmbedding(
            config.hidden_size,
            config.patch_size
        )
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                image_embeds = self.vision_embeddings(pixel_values, grid_thw=image_grid_thw) # [bs, vis_len, dim]
            ...
            if position_ids.dim() == 2:
                position_ids = position_ids[None, ...].expand(3, inputs_embeds.shape[0], -1)
...
class Qwen3ForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        # Initialize weights and apply final processing
        self.post_init()
        if getattr(config,'pretrained_weights', None) is not None:
            self.load_pretrained_weights(config.pretrained_weights)

    def load_pretrained_weights(self, ckpt):
        pretrained_model = AutoModelForCausalLM.from_pretrained(
            ckpt
        )
        self.load_state_dict(pretrained_model.state_dict(), strict=False)
        logger.info_rank0(f"Successfully initialized model weights from ckpt {ckpt}.")

        logger.info_rank0(f"Model lm_head equals to embed_token: {self.get_input_embeddings().weight == self.get_output_embeddings().weight}.")
        logger.info_rank0(f"Model lm_head ptr equals embed_token: {self.get_input_embeddings().weight.data_ptr() == self.get_output_embeddings().weight.data_ptr()}.")
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if labels is not None:
            ...
            if self.use_liger_kernel:
                ...
                loss = self.loss_fct(self.lm_head.weight, hidden_states, labels)
            else:
